...
function DisplayApointments()
{
  ...
  .then((res) => { let myObj_deserialized = res.data
  var html = ""
  for(var i=0;i<myObj_deserialized.length;i++)
  {
    html+='<li>' + myObj_deserialized[i].Name +" - " + myObj_deserialized[i].Email +" - " + myObj_deserialized[i].PhoneNumber + ' <button onclick="deleteRow('+i+')"> Delete Apointment </button>' + ' <button onclick="editRow('+i+')"> Edit Apointment </button>' + '</li>'  
  }
...
function onSubmit(e) 
{
  ...
  else 
  {
    ...
    axios.post("https://crudcrud.com/api/e8adbcd57acc460ebd939c07dfe006d2/appoitments",myNewObj).then((res) => {console.log(res.data);DisplayApointments()}).catch((err) => console.log(err))
    nameInput.value = '';
    emailInput.value = '';
    pnumberInput.value = '';   
  }
